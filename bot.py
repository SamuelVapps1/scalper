...
def resolve_watchlist(config_module) -> tuple[list, str]:
                ...
                for pos_raw in open_positions:
                    ...
                    if closed:
                        ...
                        store_risk_event(
                            {
                                "ts": updated.last_ts,
                                "type": "LOSS" if float(pnl_usdt) < 0 else "WIN",
                                "status": "TRIGGERED",
                                "reason": close_reason,
                                "symbol": updated.symbol,
                                "setup": updated.strategy,
                                "direction": updated.side,
            ...
            if symbol in watchlist_set:
                ...
                for signal in detected:
                    ...
                    intent = SignalIntent(
                        ...
                        strategy=str(signal.get("strategy", signal.get("setup", ""))),
                        reason=str(signal.get("reason", "")),
                        confidence=float(signal.get("confidence", 0.0)),
                        ts=str(signal.get("ts", signal.get("timestamp_utc", datetime.now(timezone.utc).isoformat()))),
                    )
                    bar_ts_used = str(
                        signal.get(
                            "bar_ts_used",
                    ...
                    gate_verdict, gate_reason = evaluate_intent(
                        {
                            "symbol": intent.symbol,
                            "setup": intent.strategy,
                            "direction": intent.side,
...
def _emit_scan_summary_and_heartbeat(
    ...
    *,
    ...
    now = time.time()
    idle_sec = now - _LAST_TELEGRAM_SENT_AT
...
def build_risk_autopilot(config_module):
    ...
    risk_autopilot.paper_fees_bps = config_module.PAPER_FEES_BPS
    risk_autopilot.paper_sl_atr = config_module.PAPER_SL_ATR
    risk_autopilot.paper_tp_atr = config_module.PAPER_TP_ATR
    risk_autopilot.paper_timeout_bars = config_module.PAPER_TIMEOUT_BARS
    risk_autopilot.paper_equity_usdt = config_module.PAPER_EQUITY_USDT
    risk_autopilot.risk_per_trade_pct = config_module.RISK_PER_TRADE_PCT
    risk_autopilot.max_notional_usdt = config_module.MAX_NOTIONAL_USDT
    risk_autopilot.dedup_bars = config_module.DEDUP_BARS
...
def _run_one_scan_iteration(*, config_module, risk_autopilot, dryrun_notify_always: bool) -> None:
    from storage import set_last_scan_ts

    watchlist, watchlist_mode = resolve_watchlist(config_module)
    if not watchlist:
