...
class RiskEngine:
    ...
    def assess(self, intent: Dict[str, Any]) -> RiskVerdict:
        ...
        if bool(getattr(self.settings, "kill_switch", False)):
            return self._deny("KILL_SWITCH_ON", intent)

        max_trades_day = int(getattr(self.settings, "risk_max_trades_per_day", 0) or 0)
        trade_count_today = int(self.state.get("trade_count_today", 0) or 0)
        if max_trades_day > 0 and trade_count_today >= max_trades_day:
            return self._deny("MAX_TRADES_DAY", intent)

        max_daily_loss = float(getattr(self.settings, "risk_max_daily_loss_sim", 0.0) or 0.0)
        daily_pnl_sim = float(self.state.get("daily_pnl_sim", 0.0) or 0.0)
        if max_daily_loss > 0 and daily_pnl_sim <= -max_daily_loss:
            return self._deny("MAX_DAILY_LOSS_USDT", intent)

        max_consec_losses = int(getattr(self.settings, "risk_max_consecutive_losses", 0) or 0)
        consec_losses = int(self.state.get("consecutive_losses", 0) or 0)
        if max_consec_losses > 0 and consec_losses >= max_consec_losses:
            return self._deny("MAX_CONSECUTIVE_LOSSES", intent)

        cooldown_raw = str(self.state.get("cooldown_until_utc", "") or "").strip()
        if cooldown_raw:
            try:
                ...
                if now < cooldown_until:
                    return self._deny("COOLDOWN_ACTIVE", intent)
            except ValueError:
                self.state["cooldown_until_utc"] = ""

        open_positions = [p for p in (self.state.get("open_positions", []) or []) if isinstance(p, dict)]
        open_positions = [
            p for p in open_positions
            if str(p.get("status", "OPEN") or "").strip().upper() == "OPEN"
        ]
        open_count = len(open_positions)
        max_open = int(getattr(self.settings, "max_open_positions", 1) or 1)
        if max_open > 0 and open_count >= max_open:
            return self._deny("MAX_CONCURRENT_POSITIONS", intent)

        position_mode = str(getattr(self.settings, "position_mode", "global") or "global").strip().lower()
        sym = str(intent.get("symbol", "") or "").strip().upper()
        if position_mode == "per_symbol" or bool(getattr(self.settings, "risk_one_position_per_symbol", False)):
            if sym and any(str(p.get("symbol", "") or "").strip().upper() == sym for p in open_positions):
                return self._deny("SYMBOL_ALREADY_OPEN", intent)
        if position_mode == "global" and open_count > 0:
            return self._deny("GLOBAL_ONE_POSITION", intent)

        # Preserve previous behavior: allowed intents immediately consume daily trade budget.
        self.state["trade_count_today"] = int(self.state.get("trade_count_today", 0) or 0) + 1
        self._persist_state()
        return RiskVerdict(allowed=True, reason="OK")
    ...
    def on_fill(self, trade_record: TradeRecord) -> None:
        ...
        self.store.store_risk_event(
            {
                "ts": str(trade_record.close_ts or trade_record.entry_ts or datetime.now(timezone.utc).isoformat()),
                "type": "LOSS" if pnl < 0 else "WIN",
                "status": "TRIGGERED",
                "reason": str(trade_record.close_reason or ""),
                "symbol": str(trade_record.symbol or ""),
    ...
    def _deny(self, reason: str, intent: Dict[str, Any]) -> RiskVerdict:
        self._persist_state()
        self.store.store_risk_event(
            {
                "ts": str(intent.get("ts", datetime.now(timezone.utc).isoformat())),
                "type": reason if reason in {"MAX_CONCURRENT_POSITIONS", "SYMBOL_ALREADY_OPEN", "GLOBAL_ONE_POSITION"} else "INTENT_GATE",
                "status": "TRIGGERED",
                "reason": reason,
                "symbol": str(intent.get("symbol", "") or ""),
                "setup": str(intent.get("setup", intent.get("strategy", "")) or ""),
                "direction": str(intent.get("direction", intent.get("side", "")) or ""),
            }
        )
    ...
    def _ensure_daily_rollover(self) -> None:
        ...
        state_day = str(self.state.get("day_utc", self.state.get("state_date", "")) or "")
        if state_day == today:
            return
        self.state["day_utc"] = today
        self.state["trade_count_today"] = 0
